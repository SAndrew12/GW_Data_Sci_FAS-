...
ggplot(filtered_data, aes(y = Residence, fill = Work)) +
  geom_bar() +
...
convert_narcissism <- function(score) {
  if (is.na(score)) return(NA)  # Handle NA values
  if (score == 1) {
    return("Highly Disagree")
  } else if (score == 2) {
    return("Disagree")
  } else if (score == 3) {
    return("Neutral")
  } else if (score == 4) {
    return("Kind of Agree")
  } else if (score == 5) {
    return("Highly Agree")
  } else {
    return(NA)  # Handles scores outside defined range
...
gaming_data$Narcissism_ord <- factor(gaming_data$Narcissism_ord, 
                                     levels = c("Highly Disagree", "Disagree", "Neutral", "Kind of Agree", "Highly Agree"), 
                                     ordered = TRUE)
