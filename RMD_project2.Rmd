...
#balanced_data <- SMOTE(GAD_ord ~ cleaned_whyplay + hrs_stream_combined, data = df, K = 5, perc.over = 200, perc.under = 200)

#table(balanced_data$GAD_ord)

# Train Random Forest models with different numbers of trees


#model_small <- randomForest(GAD_ord ~ cleaned_whyplay + hrs_stream_combined ,
#                             data = df, 
 #                            ntree = 500)

model_small <- randomForest(GAD_ord ~ cleaned_whyplay + Age + Gender + Hours * streams ,
                             data = df, 
                             ntree = 1)


#model_balanced <- randomForest(GAD_ord ~ cleaned_whyplay + hrs_stream_combined, 
                              # data = #balanced_data, 
                              # ntree = 500)

#model_large <- randomForest(GAD_ord ~ cleaned_whyplay + hrs_stream_combined +Gender + Age + Game,
                             #data = df, 
                             #ntree = 500)
...
#cat("Accuracy -- bal: ", mean(predict(model_balanced) == balanced_data$GAD_ord), "\n")
cat("Accuracy -- small: ", mean(predict(model_small) == df$GAD_ord), "\n")
#cat("Accuracy -- large: ", mean(predict(model_large) == df$GAD_ord), "\n")


#print(model_balanced)
print(model_small)
#print(model_large)
importance(model_small)
varImpPlot(model_small)


# Calculate counts for each GAD_ord class
...
Apply smote to balance severe anxiety
...
# Add the encoded column to your dataset
...
predictors <- df[, c("cleaned_whyplay_encoded", "Hours", "streams")]
target <- df$GAD_ord

# Apply SMOTE
set.seed(123) # For reproducibility
smote_result <- SMOTE(X = predictors, target = target, K = 5, dup_size = 4)
...
run smote again for the next minority class
...
# Select the relevant predictors and the target variable
predictors <- smote_data[, c("cleaned_whyplay_encoded", "Hours", "streams")]
target <- smote_data$GAD_ord

# Apply SMOTE
set.seed(123) # For reproducibility
smote_result <- SMOTE(X = predictors, target = target, K = 5, dup_size = 2)
...
```{r}
model_bal2 <- randomForest(GAD_ord ~ cleaned_whyplay_encoded + Hours + streams ,
