...
author: "Team 5 (Stephen Davanzo, Fyrooz Anika Khan, Anith Muthalaly) -- DATS 6101"
# date: "today"
date: "`r Sys.Date()`"
output:
  html_document:
...
df <- read.csv("G:\\My Drive\\Flash Drive Contents\\E\\Graduate School\\The George Washington University\\Data Science\\DATS 6101_12 (Morris)\\GW_Data_Sci_FAS-\\writeup_branch\\GamingStudy_data.csv")
```
...
In our EDA, we noticed that the data for the psychological inventories was not truly continuous. This was made apparent when attempting logarithmic and scale transformations. However, due to the level of difficulty this course is expecting us to master, we are assuming normality for all variables.

### Demographics

First, we need to examine the demographics of the surveyors. Since we have age, gender, country of residence, and game played, we will perform EDA on these variables.
...
The surveyors are almost entirely male. This is another, significant right-skewed variable.
...
For GADE, several respondents have indicated that they can function well in society, suggesting that the impact of generalized anxiety on their daily life is minimal. The data, similar to other variables analyzed in the EDA, is highly right-skewed.
...
Immediately we see that the total scores for all the psychological inventories need to be standardized into their respective categories, ranging from minimal to severe. Once the conversion of raw scores into ordinal values is complete, we gain a better understanding of how each psychological inventory affects gamers. The conversion is based on some research articles [@greenspace_mental_health_social_2020; @yun_satisfaction_2019; @sapra_using_2020].
...
SWL is interestingly bi-modal, with 'Slightly satisfied' and 'Slightly dissatisfied' being the most common selections chosen.
...
When looking at all of this data, a crucial component is the reason why gamers play video games in the first place. Since this field was a free-response, it would be impossible, and unwise, to display a histogram of every possible response received. In order to show meaningful results, we will utilize regular expressions to isolate key words in the responses and categorize them within specific groups.
...
We see that the data sure has anomalies because it's impossible to have 8000 hours in a week. Therefore, in order to use this variable, these extreme values, or outliers, need to be removed.

### Removing Outliers in Hours

We will use the outlierKD2 function to remove the extreme values from the Hours played variable. Once that is completed, we generate a QQ-plot, histogram, and box plot before reexamining the data distribution.
...
It is evident from the plot above that similar to the data of time spent on playing video games, the time spent on watching streams per week also has anomalies. We proceed to remove these extreme values in the next step.
...
Even after removing the outliers, we find the data will not normally distributed, as seen in the Q-Q plot. The histogram confirms this, as we find more data towards the lower tail compared to the upper tail. The long length of the box and whisker plot confirms this as well.

### Hours and Streams (Pearson's Correlation)

We can now conduct a Pearson's correlation test to measure the strength and direction of the linear relationship between Hours spent gaming and Hours spent watching streams:
...
We receive a correlation coefficient value of 0.0844. This indicates a weak, positive, linear relationship between the two variables: Hours and Streams. Since the correlation is close to 0, the relationship is weak. This means that as Hours, or time spent gaming increases, the time spent streaming increases slightly. Similarly, the inverse is also suggested.

### Standardizing Hours and Streams cleaned data

Now since the data of both the time spent on playing games and watching streams and tournaments are not normally distributed, we try to standardize them in order to proceed with the tests that assume normality.
...
We generate a Q-Q plot and histogram with the standardized values, expecting to get a normal distribution.
...
As seen from the plots above, the data now looks closer to being normalized and smoother. Now we can look into the time spent watching streams variable. 

### Streaming Q-Q plot and histogram with Standardized values

We generate a Q-Q plot and histogram again, expecting to get a bell-shaped histogram from the standardized values of the cleaned stream variable.
...
Even after standardizing the streaming time values, the plots show that the standardized values are not normalized. Both the plots confirm that the data is more concentrated towards the left side.

### Adding Hours and Streams -> Total Time, and creating histogram and QQ-plot

Previously, We have seen from the Pearson correlation test that there exists a very weak, positive correlation when we tried to find a relationship between the hours and streams variables, which are free of outliers. Even though both variables are not normally distributed, despite standardization efforts, we now try to combine the two variables to see if the summations can be used for further analysis.
...
We can clearly see from the generated Q-Q plot and histogram that the combined data, now called "Total time spent on watching streaming and playing games", looks close to completely normal. This data is now suitable to conduct all kinds of tests that require normality of data.

### Histogram of Total Time distribution on GAD_ordinal variables

We now attempt to see the distribution of total time spent on playing games and streaming based on the GAD ordinal classification: minimal/ mild/ moderate/ severe levels of anxiety. 
...
We can see from the above distributions that the hours spent playing games and watching streams is different for the distinct levels of anxiety. This brings to question if this difference is significantly different or not. We will look into this when doing hypothesis tests.
...
After performing the post-hoc test, we see that there is no actual or significant difference in hours spent streaming and hours spent playing, across all categories.
...
The GAD vs "whyplay" heat map clearly shows  that "having fun" is most closely associated "minimal anxiety", while "winning" and "escape" are more associated "moderate" and "severe" anxiety.
...
This heat map showing the residuals for GADE and "whyplay" tell a similar story to that of GAD. "having fun" is highly associated to positive mental health, in the context of anxiety disorders. "winning" and "escaping" as declared reasons why the respondents play video games, once again, are associated with negative outcomes, though the affect of "escaping" is considerably higher.
...
The heat map above for SWL vs "whyplay" shows that, again, "having fun" is most strongly associated with positive outcomes, which in this case is satisfaction with life. The response "escape" and "winning" are highly associated with respondents who are extremely dissatisfied with their life. 
...
At least one group, for example: those with mild anxiety, spends a different amount of time gaming and streaming compared to another group, for example: those with severe anxiety. Next, to understand which specific groups are different, we want to conduct post-hoc tests, like Tukey's HSD, to compare the means between each pair of GAD levels.

#### Post hoc test

The Tukey HSD test will compare the means of every possible pair of groups, for levels of GAD, to see which means are significantly different from each other.
...
The results suggest that individuals with higher levels of GAD, severe and moderate, tend to spend more combined hours on gaming and streaming compared to those with minimal anxiety. However, the difference between some intermediate levels, such as moderate vs. mild, is not statistically significant.
...
In examining the impact of anxiety levels on gaming behavior, we found that individuals with Very High anxiety levels engage in significantly more streaming hours compared to those with Low, Moderate, and High anxiety. Conversely, higher levels of gaming hours negatively affect subjective well-being, with significant declines observed at Very High hours played.

Additionally, the calculated totals of SPIN scores significantly increase from Low to Very High levels of gaming, indicating that greater gaming engagement correlates with heightened psychological distress.
...
As causality cannot be determined, we caveat this analysis by adding that pre-exiting mental health conditions, and other life stressors, could be a contributing factor to negative mental health outcomes. However, a correlative effect is clearly seen as exposed by the ANOVA and Chi-Squared tests we conducted. 

## <u>Recommendations</U>

We recommend passing our findings to social scientists to adjust the survey instruments. Collecting more comprehensive demographic information would be invaluable, including factors such as respondents' socioeconomic status (e.g., ability to afford the cost of living), family history of mental health diagnoses, and other relevant contextual data. This information could help identify potential confounding variables and provide deeper insights into the relationships we observed.
