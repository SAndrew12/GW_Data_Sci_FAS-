...
ggplot(gaming_data_clean, aes(x=gaming_data_clean$streams)) + 
  geom_boxplot() + 
  geom_boxplot(colour="orange", fill="#7777cc", outlier.colour="red", outlier.shape=8, outlier.size=4) +
...
ggplot(filtered_data, aes(x = Residence, fill = Work)) +
  geom_bar() +
  labs(title = "Bar chart depicting residences where most games 
  are played from and players's employment status",
       x = "Residence",
       y = "Count") +
  theme_minimal()
...
```{r}
ggplot(gaming_data, aes(x = GADE, fill = cleaned_whyplay)) +
  geom_bar(position = "fill") +  # Use position = "fill" to show proportions
  ylab("Proportion") +           # Label the y-axis
...
combined_data <- data.frame(Narcissism = gaming_data$Narcissism_ord,
                            Hours = hrs_GAD_filtered_clean$Hours)
...
# Adding Hours and Streams, and creating histogram

```{r}
# Create the new column hrs_stream_combined in gaming_data
gaming_data$hrs_stream_combined <- gaming_data_clean$streams + hrs_GAD_filtered_clean$Hours

# Check the structure of the updated gaming_data
str(gaming_data)
...
hist(gaming_data$hrs_stream_combined, 
...
# Hrs, Streaming_combined distribution on GAD_ordinal based
```{r}
ggplot(gaming_data, aes(x = gaming_data$hrs_stream_combined)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.7) +
  facet_wrap(~ GAD_ord) +
...
# ANOVA test for Total time spend and GAD_T
...
anova_result_combined <- aov(hrs_stream_combined ~ GAD_ord, data = gaming_data)
...
```{r}
# Perform the ANOVA
anova_result <- aov(hrs_stream_combined ~ GAD_ord, data = gaming_data)
...
# ANOVA test for Total time spent and SPIN_inventory
...
# Perform the ANOVA test
anova_result_combined <- aov(hrs_stream_combined ~ SPIN_ord, data = gaming_data)
...
Since the p-value is less than 0.05, we reject the null hypothesis. This suggests that there are significant differences in the mean combined hours spent on playing games and streaming (hrs_stream_combined) across the different levels of the Social Phobia Inventory (SPIN_ord).
...
# Perform Tukey HSD post-hoc test
tukey_result_spin <- TukeyHSD(anova_result_combined)

# Display the Tukey HSD results
print(tukey_result_spin)

# Optional: Plot the Tukey HSD results
plot(tukey_result_spin)

```
### g - Hours and Streams (Pearson's Correlation)
```{r, corr_h_s}
cor_test <- cor.test(hrs_GAD_filtered_clean$Hours, gaming_data_clean$streams)
correlation_result_Hours_Streams <- cor(hrs_GAD_filtered_clean$Hours, gaming_data_clean$streams, method = "pearson", use = "complete.obs")
cor_test
```
